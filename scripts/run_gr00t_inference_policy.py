...
class Gr00tInferencePolicy:
    ...
    def __init__(self, config: Gr00tInferenceConfig):
        ...
        # Initialize camera sensor client
        print(f"Connecting to camera server at {config.camera_host}:{config.camera_port}...")
        self.camera_sensor = ComposedCameraClientSensor(
    ...
    def _setup_joint_indices(self):
        ...
        if self.config.verbose:
            print(f"Joint index mapping:")
            for name, (start, end) in self.state_indices.items():
                print(f"  {name}: {start}-{end} ({end - start} joints)")
    ...
    def format_observation(self, state: dict, image: dict) -> dict:
        ...
        q = state["q"]  # 43-dim joint positions
        ...
        # Get ego_view image from camera
        ego_view = None
        if "images" in image:
            images = image["images"]
            # Try different possible keys for ego view
            # Camera may return as "ego_view", "ego_view/image", "ego_view_rgb", etc.
            possible_keys = [
                "ego_view", 
                "ego_view/image",  # ZMQ camera format
                "ego_view_rgb", 
                "ego_view_mono"
            ]
        ...
        # Shape: [[task_text]] for (batch=1, time=1)
        language_obs = {
