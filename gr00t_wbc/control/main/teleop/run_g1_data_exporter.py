...
class Gr00tDataCollector:
    def __init__(
        ...
        # Create subscriber BEFORE starting spin thread
        # Use a thread-safe queue since callback runs in spin thread
        from std_msgs.msg import ByteMultiArray
        self._state_queue = queue.Queue(maxsize=10)
        self._msgpack = __import__('msgpack')
        self._mnp = __import__('msgpack_numpy')
        self._state_sub = self.node.create_subscription(
            ByteMultiArray, state_topic_name, self._state_callback, 10
        )
        self.rate = self.node.create_rate(self.frequency)
        
        # Start spin thread AFTER creating subscriptions
    ...
    def _state_callback(self, msg):
        """Callback for state subscription."""
        try:
            decoded = self._msgpack.unpackb(
                bytes([ab for a in msg.data for ab in a]), object_hook=self._mnp.decode
            )
            # Use queue for thread-safe communication
            try:
                self._state_queue.put_nowait(decoded)
            except queue.Full:
    ...
    def _add_data_frame(self):
        ...
        # Show data flow status periodically
        if not hasattr(self, '_data_flow_counter'):
            self._data_flow_counter = 0
        self._data_flow_counter += 1
        if self._data_flow_counter % 100 == 1:  # Print every 100 frames
            state = "RECORDING" if self._episode_state.get_state() == self._episode_state.RECORDING else "IDLE (press 'c' to record)"
            print(f"[DATA FLOW OK] proprio: ✓ | image: ✓ | frames: {self._data_flow_counter} | state: {state}")

        if self._episode_state.get_state() == self._episode_state.RECORDING:

            # Calculate max time delta between images and proprio
            # Note: Skip time delta check when camera and control loop are on different machines
            # as their clocks aren't synchronized (one uses time.time(), other uses different base)
            max_time_delta = 0.0  # Default to 0 when timestamps aren't comparable
            ...
            for feature_name, feature_info in self.data_exporter.features.items():
                if feature_info.get("dtype") in ["image", "video"]:
                    ...
                    # Find matching image key (handle legacy naming like ego_view_rgb -> ego_view)
                    actual_key = image_key if image_key in images else None
                    if actual_key is None:
                        # Try legacy naming: ego_view_rgb for ego_view
                        legacy_key = f"{image_key}_rgb"
                        if legacy_key in images:
                            actual_key = legacy_key
        ...
        try:
            while rclpy.ok():
                ...
                with self.telemetry.timer("total_loop"):
                    # 1. poll proprio msg from thread-safe queue
